...
#2.9 Determine whether @ mention is included in the body of the message
public_msgs['has_at_mention'] = public_msgs.body.str.contains("User").astype(int)
df = public_msgs[['id','has_at_mention']]
y_data = pd.merge(y_data, df)
# visualization
...
sns.pairplot(y_data, x_vars=['msg_len','has_url', 'has_qm', 'author_age', 'has_at_mention'], y_vars='num_replies', size=6, aspect=0.7, kind='reg')
...
#4.10c Use Decision Tree with hand engineered features
from sklearn.tree import DecisionTreeClassifier
from sklearn.tree import export_graphviz
feature_cols = ['in_group', 'has_attach', 'msg_len', 'has_qm', 'has_key_word', 'author_age', 'has_at_mention']
X = y_data[feature_cols]
y = y_data['num_replies_class']
treeclf = DecisionTreeClassifier(max_depth=3, random_state=1)
treeclf.fit(X, y)
# create a GraphViz file
export_graphviz(treeclf, out_file='tree_network.dot', feature_names=feature_cols)
# compute the feature importances
pd.DataFrame({'feature':feature_cols, 'importance':treeclf.feature_importances_})
#4.10 Null Accuracy
1-y_test.mean() #.511
#4.11 Compare performance among models that use combinations of hand engineered models
from itertools import combinations
feature_cols = ['in_group', 'has_attach', 'msg_len', 'sentiment', 'has_qm', 'has_key_word', 'author_age', 'has_at_mention']
feature_cols_combos = sum([map(list, combinations(feature_cols, i)) for i in range(len(feature_cols) + 1)], [])
feature_cols_combos = feature_cols_combos[1:]
#4.11.1 Loop through logistic regression models using all combinations of feature_cols and store class prediction accuracy and AUC
class_pred_accuracy = []
y = y_data['num_replies_class']
for i in feature_cols_combos:
    X = y_data[i]
    X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=1)    
    ...
    y_pred_prob = logreg.predict_proba(extra)
    class_pred_accuracy.append(metrics.accuracy_score(y_test, y_pred_class)) 
df_model_scores = pd.DataFrame()
df_model_scores.describe()
df_model_scores['class accuracy'] = class_pred_accuracy
'''   
    class accuracy
count      255.000000
mean         0.597119
...
max          0.628512
'''
df_model_scores[df_model_scores['class accuracy'] == df_model_scores['class accuracy'].max()] #index value is 211
feature_cols_combos[211] #['has_attach', 'has_qm', 'has_key_word', 'author_age', 'has_at_mention']